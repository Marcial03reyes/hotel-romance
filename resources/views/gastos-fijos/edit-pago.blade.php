{{-- resources/views/gastos-fijos/edit-pago.blade.php --}}
@extends('layouts.app')

@section('title', 'Editar Pago - ' . $gastoFijo->nombre_servicio)
...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        ...
        <div class="flex items-center space-x-4 text-sm text-gray-600">
            ...
            <span>•</span>
            <span class="text-gray-500">Modifica la información del pago</span>
        </div>
    </div>
    ...
    <!-- Formulario -->
    <form action="{{ route('gastos-fijos.update-pago', [$gastoFijo->id_gasto_fijo, $pago->id_pago_gasto]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="id_met_pago" 
                        ...
                        @foreach($metodos as $metodo)
                            <option value="{{ $metodo->id_met_pago }}" {{ old('id_met_pago', $pago->id_met_pago) == $metodo->id_met_pago ? 'selected' : '' }}>
                                {{ $metodo->met_pago }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        ...
                        {{ $pago->comprobante ? 'Reemplazar Comprobante (Opcional)' : 'Comprobante (Opcional)' }}
                    </label>
                    <input type="file" 
                           name="comprobante" 
                    ...
                    <p class="text-xs text-gray-500 mt-1">
                        ...
                        @if($pago->comprobante)
                            <br>Si subes un nuevo archivo, reemplazará el comprobante actual
                        @endif
                    </p>
                    @error('comprobante')
                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
