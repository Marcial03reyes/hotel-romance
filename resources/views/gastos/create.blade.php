...
<div class="max-w-xl space-y-4">
    <h1 class="text-xl font-semibold">Agregar Nuevo Gasto</h1>
    ...
    <form action="{{ route('gastos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <select name="id_tipo_gasto" class="w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-blue-500" required>
                ...
                @foreach ($tipos as $t)
                    <option value="{{ $t->id_tipo_gasto }}" {{ old('id_tipo_gasto') == $t->id_tipo_gasto ? 'selected' : '' }}>
                        {{ $t->nombre }}
                    </option>
                @endforeach
            </select>
            ...
            <div>
                ...
                <select name="id_met_pago" class="w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-blue-500" required>
                    ...
                    @foreach ($metodos as $m)
                        <option value="{{ $m->id_met_pago }}" {{ old('id_met_pago') == $m->id_met_pago ? 'selected' : '' }}>
                            {{ $m->met_pago }}
                        </option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <div class="relative">
                ...
                <div class="mt-2 flex items-center text-sm text-gray-500">
                    ...
                    <span>Formatos: JPG, PNG, PDF (máximo 5MB)</span>
                </div>
                
                <!-- Preview del archivo seleccionado -->
                <div id="file-preview" class="hidden mt-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <i class='bx bx-file mr-2 text-gray-600'></i>
