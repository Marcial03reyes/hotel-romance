...
@section('title', 'Editar Gasto - Hotel Romance')
...
<head>
    ...
    <style>
        ...
        /* Alert para archivo actual */
        .current-file-alert {
            background: linear-gradient(135deg, rgba(136, 166, 211, 0.1), rgba(136, 166, 211, 0.05));
            border: 2px solid var(--tertiary-color);
            border-radius: 0.75rem;
            padding: 1rem;
            margin-bottom: 1rem;
        ...
        .current-file-alert a {
            color: var(--accent-color);
            font-weight: 600;
            text-decoration: none;
        }

        .current-file-alert a:hover {
            color: var(--secondary-color);
            text-decoration: underline;
        }

        /* Badge para ID del gasto */
        .gasto-id-badge {
    ...
    <main class="min-h-screen py-6 px-4" style="background: linear-gradient(135deg, var(--sidebar-bg) 0%, #ffffff 100%);">
        <div class="max-w-4xl mx-auto mt-1">
                    ...
                    <div class="flex items-center">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold">
                                Editar Gasto 
                                <span class="gasto-id-badge ml-2">#{{ $gasto->id_gasto }}</span>
                            </h1>
                            <p class="text-blue-100 mt-1">Modifica los detalles del gasto seleccionado</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-container">
                <form action="{{ route('gastos.update', $gasto->id_gasto) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    
                    <!-- Fila 1: Tipo de Gasto y Monto -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <!-- Tipo de Gasto -->
                        <div>
                            <label for="id_tipo_gasto" class="field-label">Tipo de Gasto *</label>
                            <select name="id_tipo_gasto" id="id_tipo_gasto" 
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-300 @error('id_tipo_gasto') border-red-500 @enderror">
                                <option value="">Seleccionar tipo...</option>
                                @foreach($tipos as $tipo)
                                    <option value="{{ $tipo->id_tipo_gasto }}" 
                                        {{ (old('id_tipo_gasto') ?? $gasto->id_tipo_gasto) == $tipo->id_tipo_gasto ? 'selected' : '' }}>
                                        {{ $tipo->nombre }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <select name="id_met_pago" id="id_met_pago" 
                                ...
                                @foreach($metodos as $metodo)
                                    <option value="{{ $metodo->id_met_pago }}" 
                                        {{ (old('id_met_pago') ?? $gasto->id_met_pago) == $metodo->id_met_pago ? 'selected' : '' }}>
                                        {{ $metodo->met_pago }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <!-- Fecha -->
                        <div>
                            <label for="fecha_gasto" class="field-label">Fecha del Gasto *</label>
                            <input type="date" name="fecha_gasto" id="fecha_gasto" 
                                   class="input-field w-full @error('fecha_gasto') border-red-500 @enderror" 
                    ...
                    <!-- Fila 3: Tipo y Código de Comprobante -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <!-- Tipo de Comprobante -->
                        <div>
                            <label for="tipo_comprobante" class="field-label">Tipo de Comprobante *</label>
                            <select name="tipo_comprobante" id="tipo_comprobante" 
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('tipo_comprobante') border-red-500 @enderror">        
                                <option value="">Seleccionar tipo...</option>
                                <option value="NINGUNO" {{ (old('tipo_comprobante') ?? $gasto->tipo_comprobante) == 'NINGUNO' ? 'selected' : '' }}>NINGUNO</option>
                        ...
                        <div>
                            ...
                            <input type="text" name="codigo_comprobante" id="codigo_comprobante" 
                                   ...
                                   value="{{ old('codigo_comprobante') ?? $gasto->codigo_comprobante }}" 
                    ...
                    <div class="mb-6">
                        <label for="comprobante" class="field-label">Archivo de Comprobante (Opcional)</label>
                        
                        @if($gasto->comprobante)
                            <div class="current-file-alert">
                                <div class="flex items-center">
                                    <i class='bx bx-paperclip text-lg mr-2'></i>
                                    <div>
                                        <strong>Archivo actual:</strong>
                                        <a href="{{ asset('storage/' . $gasto->comprobante) }}" target="_blank" class="ml-2">
                                            <i class='bx bx-link-external mr-1'></i>Ver comprobante actual
                                        </a>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="file-input">
                            <input type="file" name="comprobante" id="comprobante" 
                                   class="w-full @error('comprobante') border-red-500 @enderror"
                            ...
                            <small class="text-sm text-gray-500 mt-2 block">
                                ...
                                @if($gasto->comprobante)
                                    <br><strong class="text-orange-600">
                                        <i class='bx bx-info-circle mr-1'></i>
                                        Nota: Si subes un nuevo archivo, reemplazará el actual.
                                    </strong>
                                @endif
                            </small>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-end">
                        <a href="{{ route('gastos.index') }}" class="btn-secondary text-center">
                            <i class='bx bx-x mr-2'></i>Cancelar
                        </a>
                        <button type="submit" class="btn-romance">
                            <i class='bx bx-save mr-2'></i>Actualizar Gasto
                        </button>
                    </div>
                </form>
                ...
                <div class="info-tip mt-6">
                    <div class="flex items-start">
                        ...
                        <div class="text-sm text-gray-600">
                            <p class="font-medium mb-1" style="color: var(--accent-color);">Consejos para la edición:</p>
                            <ul class="space-y-1 text-xs">
                                <li>• Verifica que los datos modificados sean correctos antes de guardar</li>
                                <li>• El código de comprobante es obligatorio para BOLETA y FACTURA</li>
                                <li>• Si cambias el archivo, asegúrate de que sea el correcto</li>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleCodigoComprobante() {
                ...
                if (tipoSeleccionado === 'BOLETA' || tipoSeleccionado === 'FACTURA') {
                    ...
                    // No limpiar el valor en edición para preservar datos existentes
            ...
            if (fileInput) {
                fileInput.addEventListener('change', function() {
                    ...
                    if (this.files[0]) {
                        preview.innerHTML = `<i class='bx bx-check text-green-500 mr-1'></i>Nuevo archivo: ${fileName}`;
            ...
            // Efecto visual para el badge del ID
            const badge = document.querySelector('.gasto-id-badge');
            if (badge) {
