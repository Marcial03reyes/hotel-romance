...
@section('title', 'Editar Inversión - Hotel Romance')
...
<body class="text-gray-800 font-inter bg-gray-50">
    ...
    <main class="min-h-screen py-6 px-4" style="background: linear-gradient(135deg, var(--sidebar-bg) 0%, #ffffff 100%);">
        <div class="max-w-4xl mx-auto mt-1">
                    ...
                    <div class="flex items-center">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold">
                                Editar Inversión 
                                <span class="inversion-id-badge ml-2">#{{ $inversion->id_inversion }}</span>
                            </h1>
                            <p class="text-blue-100 mt-1">Modifica los detalles de la inversión seleccionada</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-container">
                <form action="{{ route('inversiones.update', $inversion->id_inversion) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="detalle" id="detalle" rows="3"
                                  ...
                                  placeholder="Describe la inversión realizada (ej: Compra de equipos de cómputo para oficina administrativa)"
                                  required>{{ old('detalle', $inversion->detalle) }}</textarea>
                        @error('detalle')
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" min="0" max="999999.99" 
                                       name="monto" id="monto" 
                                       class="input-field w-full pl-12 @error('monto') border-red-500 @enderror" 
                                       value="{{ old('monto', $inversion->monto) }}" 
                            ...
                            <select name="id_met_pago" id="id_met_pago" 
                                ...
                                @foreach($metodos as $metodo)
                                    <option value="{{ $metodo->id_met_pago }}" 
                                        {{ (old('id_met_pago', $inversion->id_met_pago) == $metodo->id_met_pago) ? 'selected' : '' }}>
                                        {{ $metodo->met_pago }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="fecha_inversion" id="fecha_inversion" 
                                   ...
                                   value="{{ old('fecha_inversion', $inversion->fecha_inversion->format('Y-m-d')) }}" required>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-end">
                        ...
                        <button type="submit" class="btn-romance">
                            <i class='bx bx-save mr-2'></i>Actualizar Inversión
                        </button>
                    </div>
                </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const badge = document.querySelector('.inversion-id-badge');
            if (badge) {
