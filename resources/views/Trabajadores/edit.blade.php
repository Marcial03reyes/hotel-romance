...
@section('title', 'Editar trabajador - Hotel Romance')
...
<div class="max-w-xl space-y-4">
    <h1 class="text-xl font-semibold">Editar trabajador</h1>
    ...
    <form action="{{ route('trabajadores.update', $trabajador->DNI) }}" method="POST" class="space-y-4">
        @csrf @method('PUT')

        <div>
            <label class="block text-sm font-medium mb-1">DNI/CE</label>
            <input name="DNI" type="text" value="{{ old('DNI', $trabajador->DNI) }}" class="w-full rounded-lg border px-3 py-2" required maxlength="20">
        </div>

        <div>
            <label class="block text-sm font-medium mb-1">Nombre y Apellido</label>
            <input name="nombre_apellido" type="text" value="{{ old('nombre_apellido', $trabajador->nombre_apellido) }}" class="w-full rounded-lg border px-3 py-2" required maxlength="100">
        </div>

        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium mb-1">Sueldo</label>
                <input name="sueldo" type="number" step="0.01" value="{{ old('sueldo', $trabajador->sueldo) }}" class="w-full rounded-lg border px-3 py-2" required>
            </div>
            <div>
                <label class="block text-sm font-medium mb-1">Fecha inicio</label>
                <input name="Fecha_inicio" type="date" value="{{ old('Fecha_inicio', \Illuminate\Support\Carbon::parse($trabajador->Fecha_inicio)->toDateString()) }}" class="w-full rounded-lg border px-3 py-2" required>
            </div>
        </div>
